...
class Item extends React.Component {
    ...
    handleChildChange(event) {
        ...
        let invalid = false;
        event.forEach((data) => {
            data.value = SanitizeName(data.value);
...
// format name so it won't break the system
function SanitizeName(name) {
    // strip whitespace and non alphanumeric characters out of string
    name = name.replace(/[^A-Z0-9]/ig, "_").replace('\'', '');
    if (name.match(/^[0-9]/)) {
        name = "_" + name;
    }
    return name;
...
function Toc() {
    const handleChange = event => {
        // strip whitespace and non alphanumeric characters out of string
        event.value = SanitizeName(event.value);
